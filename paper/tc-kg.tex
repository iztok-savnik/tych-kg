...
constructors owl:intersectionOf and owl:unionOf\footnote{The OWL union
  ...
  classes.}. The use of $\lor$-type is redundant since it can be
expressed with multiple triple types. Hence, the \emph{minimal} KR
schema language includes solely the triple types of the form
...
types given a triple $t$.

\begin{equation}
\label{rul:3-stored2}
\dfrac{t\in\upsilon,\, t=(s,p,o)\quad T_i^{i\in[1,n]}\in\tau,\, T_i=(T_s,p,T_o)\quad\/T_i^{i\in[1,n]}\in\Delta}
...
\ref{rul:min}-\ref{rul:min-gather}, which is defined to find minimal
...
from a poset of all valide stored triple types $T_i^{i\in[1,n]}$. The
algorithm implementing the operation MIN is presented in Section
\ref{sec:alg-MIN}.
