...
the domain and range types, is stored in a KG. Further, similarly to
...
exists $T\in\T$ which is a type of $t$. The second part of the
...
%The algorithm implementing the operation minimum is presented in
%Section \ref{sec:alg-MIN}.

\begin{equation}
\label{rul:3-alg-min}
\dfrac{t\in\V\quad\vdasharr\/t:_\uarr\lor[T_i^{i\in[1,n]}]\quad\vdasharr\/\{S_j^{j\in[1,m]}\}=\,\Darr\kern -3pt[T_i^{i\in[1,n]}]}
