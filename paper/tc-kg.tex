...
\begin{equation}
\dfrac{t:_\darr\land[T_1..T_n]\quad T\in\Tt\quad \forall\/i\in[1..n], T_i\preceq\/T\quad \forall\/S\in\T_t,\forall\/i\in[1..n], T_i\preceq\/S\land\/T\preceq\/S}
      {\vdash\/t:_\sqcup\/T}
\end{equation}
...
\begin{equation}
\label{rul:3-stored}
\dfrac{t\in\Ti,t=(s,p,o)\quad p'\in\I_p, p\preceq\/p'\quad (p',\text{domain},T_s)\in\/g\quad (p',\text{range},T_o)\in\/g}
      {t:_\uarr(T_s,p,T_o)}
\end{equation}
...
\begin{equation}
\dfrac{\forall(T_i\in\T_t), t:_\uarr\/T_i\quad \forall(t:_\uarr\/S), T_i\preceq\/S\lor\/T_i\not\sim\/S}
      {t:_\sqcap\lor[T_i]}
\end{equation}

The first premise says that we identify all triple types $T_i$ that
are the types of $t$. The second premise says that for each $T_i$, all
types $S$ of $t$ are either more general or equal, or not related to
