...
\subsection{The join and meet typing}
...
In a KG we have a poset of (stored) types. that can be used to compute
a join type $J=S\sqcap\/T$ which is the least type $J$ such that
$S\preceq\/J$ and $T\preceq\/J$, and a meet type $M=S\sqcup\/T$ which is
the greatest type $M$ such that $M\preceq\/S$ and $M\preceq\/T$. Since
we have a top type $\top$ defined in a KG, the join of arbitrary two
types always exists. However, the meet of two arbitrary types may not
exist always.

\notes{General rules for join and meet.}
\notes{Join and meet as typing (implementing) of either $\land$ or $\lor$ types.}
\notes{Application of the join on ground $\land$-types.}
\notes{}  
