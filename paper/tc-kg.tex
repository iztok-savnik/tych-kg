...
\notes{In the case of indirect definition of a triple type the domain and range is inherited from the properties of super-predicates.}
\notes{Stored types of triples are in a KG represented with the predicates rdfs:domain and rdfs:range.}
\notes{Formally, the triple types $T_s*p*T_o$ are represented as triples that inlcude solely a predicate and two types.}
...
definition of a predicate with multiple meanings, depending on the
context of the predicate. This can happen when multiple ontologies are
merged into one KG. Another reason for adding $\lor$-types to a KG can
be the expressive power of KG. These KGs use owl:unionOf and
...
model transformation tools. Finally, KGs that can define contexts and
use contextual reasoning also use $\land$ and $\lor$-types. Examples
of such KGs are Cyc \cite{cyc}, Scone \cite{Fahlman2011} and RACER
\cite{Haarslev2012}.

We have two ways of modelling predicate sense. The first is by using
one predicate for one meaning. A current trend in KGs is to model each
sense of a predicate with a separate name. The general form of
predicate can be specialized to sub-predicates modelling different
senses. In the case that predicates have many senses
(homophones,polysemy) this representation can lead to predicate
explosion. The reasoning is simpler if all predicates have unique
meaning. But the predicates are context insensitive.

The second is that we have a predicate that can be defined differently in different contexts (microtheories, ontologies).
Whitin an ontology it seems that the triple type can be used, quite naturally, as the context of a predicate.
The meaning of predicate is modelled by a triple type including concrete classes, types of domains and ranges.
We have one triple type (predicate and types of S and O) mapped to one meaning.
The representation requires using contextual reasoning \cite{cyc}.
One of the pros is that contextual representation and reasoning is close to the natural language \cite{Fahlman2011}.
Pitfalls of word sense disambiguation and contextual reasoning....
...
stored type is constructed by selecting triple types that are composed
of a predicate $p$ and the domain and range types linked to predicates
$p'\succeq\/p$. We say that the domain and range types are
\emph{inherited} from the predicates $p'\succeq\/p$.
...
The above rule gathers the types of the domain and range of a
...
different predicates $p_1$ and $p_2$ "above" $p$ ($p_1,p_2\succ\/p$)
which are comparable, i.e., $p_1\sim\/p_2$. This condition restricts
the domain and range to be defined on the same path from $p$ to some
maximal element $m$ of the predicate poset. In the second part of the
premise the domain and range types are gathered for the selected
predicates $p_1$ and $p_2$, respectively.
...
\dfrac{t\in\T_i\quad T\in\tau,\, t:_\uarr\/T\quad \forall\/S\in\tau,\, t:_\uarr\/S\quad T\preceq\/S\lor\/T\not\sim\/S}
      {t:_{\Darr}\/T}
\end{equation}
...
$t$. In other words, there is no type $S$ of $t$ that is a subtype of
$T$. Hence, $T$ is the minimal type of the stored triple types of $t$.
...
the definition of multiple senses of a predicate. There are multiple
reasons to use the contextual representation and reasoning in a
KG. First, with the evolvment of KGs there are many cases where the
RDF store is represented in the modular way spliting the dataset into
parts that corespond to the contexts. The meaning of a predicate can
be different in different contexts while the reasoner is able to
disambiguate among the different meanings of a predicate. There are
many ways to define contexts in practical systems. Examples are the
named graphs in KGs DBpedia, Wiki Data and Yago. Another example is
Cyc \cite{cyc} that uses microtheories to represent different
contexts. Similarly to Cyc, Scone \cite{Fahlman2011} is a KR system
that can define spaces (contexts) and uses contextual
reasoning. Finally, the collective findings of the research in the
area of Cognitive science \cite{Hollister2017} show that natural
language is inherently contextual, and the context is essential in the
human representation of knowledge and reasoning.

In mathematics, a function is not represented by its name only, but
also with the types of its domain and range. Types of function can
disambiguate among the different functions with the same name but
different domain and range types. Similarly, the types can be
effectively used to disambiguate the meaning of the predicate in a KG.

To be able to study KRs including predicates with multiple senses we
propose the minimal KR schema language that includes the triple types
in the form of triples (instead of specifying the domain and range of
predicate) together with the OWL type constructors owl:intersectionOf
and owl:unionOf. These are often used for extending the semantics of a
KG \cite{}. The triple types are stored in a KG as triples composed of
the types of S and O components and the predicate. The S and O
components can be classes or type constructors applied to classes.

In the data model of RDF-Schema the context of predicate is the
predicate itself. However, it is common in 

A stored triple type of $t$ is a disjunction of a set of minimal
elements from a set of all valid stored triple types of $t$. The
minimal stored types are gathered in a $\lor$-type. 

\begin{equation}
\label{rul:gather-min-types}
\dfrac{t\in\upsilon\quad\/T_i^{i\in[1,n]}\in\tau\quad \forall\/i\in[1,n],\ t:_\Darr\/T_i}
      {t:_\Darr\lor[T_i]}
\end{equation}

The premise says that we identify all triple types $T_i$ that are the
MIN types $t:_\Darr\/T_i$. The minimal stored triple types are
gathered in a triple type $\lor[T_i]$.

The following Rule \ref{rul:min-stored-type-alg} presents the
algorithmic typing of minimal types of a poset. All stored types of
$t$ are first gathered in a set ordered by $\preceq$. The minimal
elements of poset are selected using the operator $\Darr$ defined as a
function. Finally, the computed minimal elements are gathered in a
$\lor$-type.

\begin{equation}
  \label{rul:min-stored-type-alg}
\dfrac{t\in\upsilon\quad\/T_i^{i\in[1,n]}\in\tau\quad \vdasharr\forall\/i\in[1,n],\ t:_\uarr\/T_i\quad \vdasharr\{S_j^{j\in[1,m]}\} =\,\Darr\kern -3pt[T_i^{i\in[1,n]}]}
{\vdasharr\/V:_{\Darr}\lor[S_i^{i\in[1,m]}]}
\end{equation}
