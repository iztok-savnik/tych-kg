...
\ennotes{The following rule geathers all ground types of $I\in\I_i$.}
...
\notes{Let's have a look at $\land$ types of $I$'s ground types $S_1..S_n$ from a KG. }
\nnotes{Often $I$ has a set of very specific classes $C_s$ but also some general classes $C_g$.}
\nnotes{The general classes $C_g$ are close to the classes used in the stored triple types.}
\nnotes{If stored typing of $I$ is correct, then $s_s$ have to include subclasses of $s_g$.}

\notes{The ground type $\land[S_1..S_n]$ can include pairs of types $S_i\preceq\/S_k$.}
\nnotes{It makes sense either to compute MIN or MAX of $\land[S_1..S_n]$ yielding $\land[U_1..U_m]$.}
\nnnotes{$m\le\/n$ and $U_j^{j\in[1,m]}\in\{S_i^{i\in[1,n]}\}$.}

\notes{The operator MIN retains from $\land[S_1..S_n]$ only the most specific classes.}
\nnotes{The following is teh definition of the operation MIN.}
\nnotes{The MIN operation is applied to $\land[S_1..S_n]$ obtaining $\land[U_1..U_m]$.}
\nnnotes{$\land[U_1..U_m]$ includes types $U_j^{j\in[1,m]}\in\{S_k^{k\in[1,n]}\}$ such that $\not\exists\/S_i^{i\in[1,n]}(S_i\prec\/U_j)$.}
\nnnotes{All pairs of classes $C_1,C_2\in\{U_j^{j\in[1,m]}\}$ are \emph{incomparable},}
\nnnotes{i.e., $C_1\not\sim\/C_2\equiv\/C_1\not\preceq\/C_2\land\/C_1\not\succeq\/C_2$.}
\ennotes{The logical rule for the operation MIN is as follows.}

\begin{equation}
\label{rul:min}
\dfrac{I\in\I_i\quad\/I:_\darr\land[S_1..S_n]\quad\/U\in\/\{S_k^{k\in[1,n]}\}\quad\forall\/i\in[1,n],\ U\preceq\/S_i\lor\/U\not\sim\/S_i}
      {I:_{\Darr}U}
\end{equation}

The rule says that $U$ is a minimal type of a ground type
$\land[S_1..S_n]$. $U$ is minimal since all other $S_i$ types are
either more general or equal ($\succeq$), or not related to $U$. The
rule generates all MIN types of $\land[S_1..S_n]$. The rule for
filtering $\land[S_1..S_n]$ all $S_i\succeq\/S_k$ by using algorithmic
typing is defined as follows.

\begin{equation}
\label{rul:min-alg}
\dfrac{I\in\I_i\quad\vdasharr\/I:_\darr\land[S_1..S_n]\quad\vdasharr\/U=\,\Darr\kern -3pt[S_1..S_n]}
      {\vdasharr\/I:_{\Darr}U}
\end{equation}

The following rule is used for gathering all MIN types of
$\land[S_1..S_n]$. The result is a conjunction of minimal types
...
$t$. Therefore,
$\llbracket\land[U_1..U_m]\rrbracket\subseteq\llbracket\land[S_1..S_n]\rrbracket$.

\begin{equation}
\label{rul:min-gather}
\dfrac{I\in\I_i\quad\forall\/i\in[1,m],\ I:_\Darr\/U_i}
      {I:_{\Darr}\land[U_1..U_m]}
\end{equation}

\notes{For $I\in\I_i$ compute a join type of $S_1..S_n$ as $I:\sqcup[S_1..S_n]$.}
\nnotes{A join type is defined on the set of types $\{S_i^{i\in[1,n]}\}$.}
\nnotes{A join type $S=\sqcup[S_1..S_n]$ is the LUB type of $\{S_i^{i\in[1,n]}\}$ with respect to the relation $\preceq$.}
\nnotes{$\sqcup[S_1..S_n]$ is a minimal type $S$ that is related to all types $S_i^{i\in[1,n]}$ via $\preceq$.}
...
\begin{equation}
\label{rul:lub-logic}
\dfrac{I\in\I_i\quad\/I:_\darr\land[S_1..S_n]\quad\/S\in\I_c\quad\forall\/T\in\I_c,\ \forall\/i\in[1..n],\ (S_i\preceq\/T\land\/S\preceq\/T)\lor\/S\not\sim\/T}
      {I:_\sqcup\/S}
\end{equation}

\begin{equation}
\label{rul:lub-alg}
\dfrac{I\in\I_i\quad\/I\in\I_i\quad\vdasharr\/I:_\darr\land[S_1..S_n]\quad\/\vdasharr\/S=\sqcup[S_1..S_n]}
      {\vdasharr\/I:_\sqcup\/S}
\end{equation}
