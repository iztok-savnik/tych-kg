...
%% KG data model - representation language
...
% on static structure of a KG
\notes{What is structural part of KG? Describe briefly.}
\notes{The values of KG are complex because of the rich modelling contructs of a data and knowledge representation language.}
\notes{The static structure of a KG is built from the identifiers that are bound into triples.}
\notes{The identifiers and triples represent the basic structures of a KG, i.e., the values from languages that work with KGs (basic graph-patterns, SparQL, ...).}
\nnotes{The structures can be comared to OO world of classes and objects, or in some respect also to record (class) types and records.}
\notes{A KG includes triples that represent the values and user-defined types that form the schema of a KG \cite{Savnik2022}.}
\nnotes{All ground triples are typed (?). However, the user-defined type of a triple is not linked directly to a triple.}
...
\notes{Hence, the types of ground triples must be derived from a KG by selecting the appropriate user-defined triple types from the types of components.}
...
\notes{As a consequence, before we can define typing of languages that work with a KG, we have to be able to type a triple.}

% abstract of type checking KG ground triples
\notes{We divide triples from a KG into ground triples and triples that represent triple types.}
\notes{In order to check the types of a KG, the type of each ground triple has to be checked.}
\notes{There are two way of computing the type of a triple.}
...
\notes{Type checking of a ground triple is converted to checking the relationships between the ground type and user-defined type of a triple.} 
...
\notes{For $V\in\upsilon$ gather ground types of identifiers with $\land$-type as $V:_{\darr}\land[T_1..T_n]$.}
\nnotes{$V\in\upsilon\quad\forall\/T_i\in\tau,\ t:_{\darr}T_i$.}
\nnotes{The ground type of $V$ is a type $T_g=\land[T_1..T_n]$.}
...
\dfrac{V\in\upsilon\quad\/T_i^{i\in[1,n]}\in\tau\quad\/V:_{\darr}T_i^{i\in[1,n]}}
      {V:_{\darr}\land[T_1..T_n]}
\end{equation}

\notes{Let's have a look at $\land$ types of $V$'s ground types $T_1..T_n$ from a KG. }
\nnotes{Often $V$ has a set of very specific classes $C_s$ but also some general classes $C_g$.}
\nnotes{The general classes $C_g$ are close to the classes used in the stored triple types.}
\nnotes{If stored typing of $V$ is correct, then $s_s$ have to include subclasses of $s_g$.}

\notes{The ground type $\land[T_1..T_n]$ can include pairs of types $S_i\preceq\/S_k$.}
\nnotes{It makes sense either to compute MIN or MAX of $\land[T_1..T_n]$ yielding $\land[U_1..U_m]$.}
\nnnotes{$m\le\/n$ and $U_j^{j\in[1,m]}\in\{T_i^{i\in[1,n]}\}$.}

\notes{The operator MIN retains from $\land[T_1..T_n]$ only the most specific classes.}
\nnotes{The following is teh definition of the operation MIN.}
\nnotes{The MIN operation is applied to $\land[T_1..T_n]$ obtaining $\land[U_1..U_m]$.}
\nnnotes{$\land[U_1..U_m]$ includes types $U_j^{j\in[1,m]}\in\{T_k^{k\in[1,n]}\}$ such that $\not\exists\/T_i^{i\in[1,n]}(T_i\prec\/U_j)$.}
\nnnotes{All pairs of types $S_1,S_2\in\{U_j^{j\in[1,m]}\}$ are \emph{incomparable},}
\nnnotes{i.e., $S_1\not\sim\/S_2\equiv\/S_1\not\preceq\/S_2\land\/S_1\not\succeq\/S_2$.}
\ennotes{The logical rule for the operation MIN is as follows.}

\begin{equation}
\label{rul:min}
\dfrac{V\in\upsilon\quad\/V:_\darr\land[T_1..T_n]\quad\/U\in\/\{T_k^{k\in[1,n]}\}\quad\forall\/i\in[1,n],\ U\preceq\/T_i\lor\/U\not\sim\/T_i}
      {V:_{\Darr}U}
\end{equation}

The rule says that $U$ is a minimal type of a ground type
$\land[T_1..T_n]$. $U$ is minimal since all other $T_i$ types are
either more general or equal ($\succeq$), or not related to $U$. The
rule generates all MIN types of $\land[T_1..T_n]$. The rule for
filtering $\land[T_1..T_n]$ all $T_i\succeq\/T_k$ by using algorithmic
typing is defined as follows.

\begin{equation}
\label{rul:min-alg}
\dfrac{V\in\upsilon\quad\vdasharr\/V:_\darr\land[T_1..T_n]\quad\vdasharr\/U=\,\Darr\kern -3pt[T_1..T_n]}
      {\vdasharr\/V:_{\Darr}U}
\end{equation}

The following rule is used for gathering all MIN types of
$\land[T_1..T_n]$. The result is a conjunction of minimal types
$\land[U_1..U_m]$. The interpretation of a minimal ground type
$\llbracket\land[U_1..U_m]\rrbracket$ of $t$ is minimal since
$\land[U_1..U_m]$ includes a set of minimal and unrelated types of
$t$. Therefore,
$\llbracket\land[U_1..U_m]\rrbracket\subseteq\llbracket\land[T_1..T_n]\rrbracket$.

\begin{equation}
\label{rul:min-gather}
\dfrac{V\in\upsilon\quad\forall\/i\in[1,m],\ V:_\Darr\/U_i}
      {V:_{\Darr}\land[U_1..U_m]}
\end{equation}

\notes{For $V\in\upsilon$ compute a join type of $T_1..T_n$ as $V:\sqcup[T_1..T_n]$.}
\nnotes{A join type is defined on the set of types $\{T_i^{i\in[1,n]}\}$.}
\nnotes{A join type $T=\sqcup[T_1..T_n]$ is the LUB type of $\{T_i^{i\in[1,n]}\}$ with respect to the relation $\preceq$.}
\nnotes{$\sqcup[T_1..T_n]$ is a minimal type $T$ that is related to all types $T_i^{i\in[1,n]}$ via $\preceq$.}
\nnotes{$\llb\land[T_1..T_n]\rrb\subseteq\bigcup_{i\in[1..n]}\llb\/T_i\rrb\subseteq\llb\/T\rrb$.}
\ennotes{A join type is used when we want to determine the user-defined type of a triple $t$.}

\begin{equation}
\label{rul:lub-logic}
\dfrac{V\in\upsilon,\, \/V:_\darr\land[T_1..T_n]\quad\/T\in\tau,\, T_i^{i\in[1,n]}\preceq\/T\quad\forall\/S\in\tau,\, (S_i^{i\in[1,n]}\preceq\/S\land\/T\preceq\/S)\lor\/S\not\sim\/T}
      {V:_\sqcup\/T}
\end{equation}

\begin{equation}
\label{rul:lub-alg}
\dfrac{V\in\upsilon\quad\vdasharr\/V:_\darr\land[T_1..T_n]\quad\/\vdasharr\/T=\sqcup[T_1..T_n]}
      {\vdasharr\/V:_\sqcup\/T}
\end{equation}
...
\nnotes{$t:_\sqcup\/S$ and there should be a path from $S$ to class components of stored triple types $T$.}
...
\subsection{Computing stored MIN types}
