...
A KG stores predefined classes that are organized into taxonomies
\cite{Baader2002}. Further, typing of ground identifiers, as well as
typing of the predicates with the domain and range types, is also
stored in a KG. Similarly to the \emph{roles} \cite{Brachman1985} of a
knowledge base, the predicates are treated as classes. As classes,
predicates are organized into a taxonomy. However, they also act as
individual entities described with additional predicates. Finally, the
predicates are inherited through the taxonomy of classes similarly as
data members and methods are inherited among classes of
object-oriented programming languages. And since predicates also act
as classes, the predicates of the predicates, like the domain and
range of a predicate, are inherited through the taxonomy of
predicates.
...
predicate name. Further, the $\land$-$\lor$ types \cite{Pierce1996}
...
operators. In the proposed type system, we use the operators defined
on posets in rules. Examples include the operator \emph{minimum},
which, given a poset, returns a set of minimal poset elements, and the
operator for computing \emph{minimal upper bounds} of a subset of
elements from a poset \cite{DaveyPriestley2002}.
...
and encompasses a complete KG when typing a ground triple. Second, the
...
analysis of typing a ground triple spans from the ground types at the
lower levels of ontology to the minimal schema types at the upper
...
type identifiers $\TT_i$, i.e., types of ground identifiers $\V_i$,
and the triple types $\TT_t$, i.e., the types of ground triples
$\V_t$.  Finally, a set $\TT_c\subseteq\TT_i$ is a set of type (class)
identifiers, and a set $\TT_p\subseteq\TT_i$ denotes a set of
predicates that now have the role of types.
...
typing and subtyping judgements, expressions of the FOL, or set
definition statements. FOL expressions can express complex premises,
such as the conditions for the LUB types. The conclusion part of the
rule is a typing or subtyping judgment that can include a set
...
The typing expression $V:_1\/T$ denotes a \emph{stored typing}
relation $:_1$ that links a value $V\in\V_i$ to a ground type
identifiers $T\in\TT_i$. The stored typing is a one-step typing
relation based on the triples stored in a KG.

The stored types are linked to the ground identifiers via stored
triples $(I,\text{rdf:type},T)\in\Delta$. Consequently, we can write
$I:_1\/T$. The rule for the one-step stored typing relation $:_1$ is
defined as follows.
...
The individual entity $I$ can have more than one stored type. 
Using Rule \ref{rul:ident-1step-type} as a generator gives 
all types $T_j^{j\in[1,n]}$ such that $I:_1\/T_j^{j\in[1,n]}$.

If we want to obtain all valid stored types of $I$, Rule
\ref{rul:ident-1step-type} can be used either in some other rule that
employs it as a generator, or we can update the above rule to generate
an $\land$-type, including all the stored types of $I$ as presented in
Section \ref{sec:intsc-union}.

The one-step subtyping relation $\preceq_1$ is defined on
...
$\preceq_1$ on the predicates $\TT_p$ is based on the RDF-Schema
...
The one-step relation $\preceq_1$ is extended with the
...
First, the one-step relation $\preceq_1$ is generalized to the
relation $\preceq$ defined over type identifiers $\TT_i$.

\begin{equation}
\label{rul:Ic-extended}
\dfrac{I_1,I_2\in\TT_i \quad I_1\preceq_1\/I_2}
      {I_1 \preceq I_2}
\end{equation}
...
The one-step typing relation $:_1$ is generalized to the typing
relation $:$ with the following rule.

\begin{equation}
\dfrac{I\in\V_i\quad\/T\in\TT_i\quad\/I:_1\/T}
      {I:T}
\end{equation}

Note that a unique notation for stored typing allows us to address
...
set of the MLB types $M_j^{j\in[1,m]}\in\/M$
such that $M_j$ is a lower bound with $M_j\preceq\/T_i^{\s\/i=1..n}$,
and all other lower bounds $U$ with $U\preceq\/T_i^{\s\/i=1..n}$
entail $U\preceq\/M_j$. Note that the meet of the set of types from a
KG does not always exist.
...
$$\llb\lor_{i=1}^n\/T_i\rrb_\DD=\bigcup_{i=1}^n\llb\/T_i\rrb_\DD\subseteq\bigcap_{j=1}^m\llb\/J_j\rrb_\DD=\llb\sqcup_{i=1}^n\/T_i\rrb_\DD$$

While the interpretation of the type $\lor_{i=1}^nT_i$ includes
precisely the instances of all $T_i$, the interpretation of the types
$J_j\in\/J=\sqcup_{i=1}^n\/T_i$ contains interpretations of
$T_i^{i=1..n}$ since $T_i^{i=1..n}\preceq\/J_j^{i=1..m}$. Hence,
$\cup_{i=1}^n\llb\/T_i\rrb_\DD\subseteq\cap_{j=1}^m\llb\/J_j\rrb_\DD$. The
interpretation of $J_j^{j=1..m}$ can include besides the
interpretations of $T_i^{\s\/i=1..n}$ also the interpretations of some
other classes.
...
such that $L\preceq\/T_i^{i=1..n}$. The meet types are not frequent
on the lower levels of a class ontology from a KG.

As in the case of the $\lor$-type and the join type, the semantics of
the $\land$-type is comparable to the semantics of the meet type. An
$\land$-type is a type that implements logical view of the greatest
lower bound type. The meet types are based on the concrete poset of KG
types and represent concrete types. Their interpretation is contained
in the interpretation of a $\land$-type. The type $\land_{i=1}^n\/T_i$
denotes the intersection $\bigcap\/\llb\/T_i\rrb_\DD$ while the
interpretation of a meet types
...
approximated by using join and meet types to obtain a \emph{concrete}
...
\subsection{Typing identifiers with $\land$-$\lor$ types\label{sec:typing-and-or}}

In this section, we first gather the classes that are the ground types
of an individual (ground) identifier $I\in\V_i$ by using an
$\land$-type $T$ in Rule \ref{rul:land-1}. Since a user defines the
stored ground types of a ground identifier, the stored types may
include subclasses of other stored types. Therefore, the minimal type
$T_{min}$ is first determined by eliminating the redundant class
identifiers from $T$ in Rule \ref{rul:min}. Finally, from the minimal
ground type $T_{min}$ of $I$ we derive the minimal upper bound (MUB)
by Rule \ref{rul:mub}. The MUB type is the starting point for
investigating the relations between the ground type and the schema
type of a triple $t$ as presented in Section \ref{sec:triples}.
...
\begin{equation}
\label{rul:land-1}
\dfrac{I\in\V_i\quad\/T_i^{\s\/i=1..n}\in\TT_i\quad\/I:T_i^{\s\/i=1..n}}
      {I:\land_{i=1}^n\/T_i}
\end{equation}
...
Given a set of types $P=\{T_i\}_{i=1}^n$ ordered by $\preceq$, the
minimal elements of $P$ are the elements
$S_j^{\s\/j=1..m}\in\{T_i\}_{i=1}^n$ such that
${\nexists}\/T_i^{\s\/i=1..n}(T_i\prec\/S_j)$. All pairs of types
...
\begin{equation}
\label{rul:min}
\dfrac{I\in\V_i\quad\/I:\land_{i=1}^n\/T_i}
      {I:_\Darr\land \{S \mid S\in\/\{T_i\}_{i=1}^n\land\forall\/i\in[1,n],\ S\preceq\/T_i\lor\/S\not\sim\/T_i\}}
\end{equation}


Note that we used the annotated typing relation "$:_\Darr$"
to denote that we would like to infer the minimal type of $I$.

\memo{
  The rule says that the types $S$ gathered with the $\land$-type are
the minimal types of a poset of ground types $\{T_i\}_{i=1}^n$. A
particular type $S$ is minimal since all other $T_i$ types are either
more general or equal ($\succeq$), or not related to $S$.}

  
Let us now present the typing rule that, given $I\in\V_i$ and
$I:_\darr\land_{i=1}^n\/T_i$, derives the join ground type of $I$. Recall
from Section \ref{sec:join-meet-types} that we defined the operation
join as the MUB of a set $\{T_i\}_{i=1}^n$. A join
$\sqcup_{i=1}^n\/T_i$ is a set of minimal upper bounds
$\{S_j\}_{j=1}^m$ that are related to all types $T_i^{\s/i\in[1,n]}$
via $\preceq$, and are minimal. Below is the logical definition of the
join rule.

\begin{equation}
\label{rul:mub}
\dfrac{I\in\V_i,\, \/I:\land_{i=1}^n\/T_i}
      {\begin{array}{l}
        I:_\sqcup\land\{S \mid \/S\in\TT_i,\,\/T_i^{\s\/i=1..n}\preceq\/S\ \land \\
        \phantom{I:\land\{S \mid\ }\forall\/P\in\TT_i,\,((T_i^{\s\/i=1..n}\preceq\/P\land\/S\preceq\/P)\lor\/P\not\sim\/S)\}
       \end{array}}
\end{equation}

The annotated typing relation $:_\sqcup$ is used in the conclusion of
the above rule to be able to explicitly require the use of the rule
in other rules. For example, Rule \ref{rul:mub} is used for the
definition of Rule \ref{rul:3-join}, which is used for the derivation
of the join ground type of a ground triple.

The join ground types are used as the starting points for searching a
correct schema type. In case a predicate has two alternative
definitions in two different contexts, then a path from an MUB type to
the schema of a KG determines the corresponding schema type. The
details are presented in Section \ref{sec:typing-3ples}.
...
This section presents the typing of triples. First, in Section
\ref{sec:3-types} we describe the product type and its role as a
triple type. In the following sub-sections, the rules for typing a
ground triple $t=(s,p,o)$ are presented. We use two different types to
describe $t$. The \emph{ground type} of $t$ is based on the stored
typing of the ground identifiers. The rules for deriving the ground
type are presented in Section \ref{sec:3-ground-types}. The
\emph{schema type} of $t$ is derived from the stored conceptual schema
of $t$. The rules for the derivation of the schema type are given in
Section \ref{sec:schema-3types}. To be able to distinguish the rules
for the derivation of a ground type from the rules for deriving the
schema type of $t$, we denote the ground typing relation as $:_\darr$,
and the schema typing relation as $:_\uarr$. After computation of the
ground and schema types of $t$ the final type of $t$ is determined by
relating the schema and ground types. This is presented in Section
\ref{sec:typing-3ples}.
...
A type of a triple $t=(s,p,o)\in\V_t$ is a product type $D*p*R$
...
$$(\land_{i=1}^n\/D_i, p ,\land_{j=1}^m\/R_j).$$
...
related to $I$ by one-step type relationship $:_1$, as presented
...
A ground type of a triple $t=(I_s,p,I_o)$ is a product type $S*p*T$
that we represent as a triple $(S,p,T)$. A triple type includes the
ground types of $t$'s components $I_s$ and $I_o$, and the property
$p$, which now has the role of a type. The following rule defines a
ground type of a triple.

\begin{equation}
\label{rul:3-ground}
\dfrac{\begin{array}{c}
       t\in\V_t,\ t=(I_s,p,I_o) \\
       S,T\in\TT_i\quad I_s:S\quad I_o:T\quad p:\text{rdf:Property}
       \end{array}}
       {t:_\darr(S,p,T)}
\end{equation}
...
\dfrac{\begin{array}{c}
       t\in\V_t,\, t=(I_s,p,I_o) \\
       S,T\in\TT_i\quad I_s:_\Darr\/S\quad I_o:_\Darr\/T\quad p:\text{rdf:Property}
       \end{array}}
      {t:_\darr\/(S,p,T)}
\end{equation}
...
typing relation "$:$", which can be used instead of any annotated typing
relation.
...
for the subject and object components of $t$, respectively, then also
the triple types from the conclusion of the rule are valid.
...
\dfrac{\begin{array}{c}
       t\in\V_t,\ t=(I_s,p,I_o) \\
       S,T\in\TT_i\quad I_s:_\sqcup\/S\quad I_o:_\sqcup\/T\quad p:\text{rdf:Property}
       \end{array}}
      {t:_\darr\/(S,p,T)}
\end{equation}

The join type $(S,p,T)$ includes in the subject and object components
the $\land$-types composed of one or multiple MUB classes, i.e.,
$(\land_{i=1}^{n}S_i,p,\land_{j=1}^{m}T_j)$. We convert this type
using Rule \ref{rul:3-land-transform} into a conjunction of basic MUB
triple types $M=\land_{i=1..n, j=1..m}(S_i,p,T_j)$ to be used as a
\emph{final ground type}.
...
\subsection{Schema triple types\label{sec:schema-3types}}
...
languages. In Section \ref{sec:schema-3types}.1 we present the
derivation of schema types when the RDF-Schema is used. Further, in
Section \ref{sec:schema-3types}.2 we define the typing rules for
...
\dfrac{\begin{array}{c}
       ...
       T\in\TT_c,\, (p_1,\text{domain},T)\in\/\Delta \\
       S\in\TT_c,\, (p_2,\text{range},S)\in\Delta
       \end{array}}
      {t:_\uarr\/(T,p,S)}
\end{equation}

The above rule generates valid schema types $(T,p,S)$ for a given
predicate $p$. We allow that a domain and range types are defined for
different predicates $p_1,p_2\succeq\/p$ since such cases appear in a
KG. However, we have to be careful that the rules of inheritance are
respected. We can only inherit from $p_1$ and $p_2$, which are related
by a subtype relation: $p\preceq\/p_1\preceq\/p_2$. This condition
restricts the domain and range to be defined on the same path from $p$
to some maximal element $m$ of the predicate $p$ poset.

Rule \ref{rul:3-stored1} generates all valid schema types of
...
logical judgment for a minimal schema type of $t$. 

\begin{equation}
\label{rul:min-stored-type}
\dfrac{\begin{array}{c}
         t\in\TT_i\quad T_i^{i=1..n}\in\TT_t,\, t:_\uarr\/T_i \\
         S_j^{j=1..m}\in\TT_t,\, t:_\uarr\/S_j\quad T_i\preceq\/S_j\lor\/T_i\not\sim\/S_j
       \end{array}}
      {t:_{\uarr}\land_{i=1}^n\/T_i}
\end{equation}

The first part of the premise says that $t$ is a ground triple and
there exists $T_i^{i=1..n}\in\TT_t$, which are the types of $t$. The
second part of the premise requires that $T_i^{i=1..n}$ are the
minimal types. In other words, there are no such types $S_j^{j=1..m}$
of $t$ that are subtypes of $T_i^{i=1..n}$. Hence, $T_i^{i=1..n}$ are
the minimal types of the stored triple types of $t$.

Note that if the schema is defined by using RDF-Schema, and the stored
schema typing is correct, then $n=m=1$ and the condition
$T_i\preceq\/S_j$ is $true$. Consequently,
Rule~\ref{rul:min-stored-type} generates exactly one minimal type.
...
through the RDF Schema. On the other hand, the contextual
...
\subsection{Typing a triple\label{sec:typing-3ples}}
...
errors in types from the domain and/or range of a predicate. In
...
defined in Section \ref{sec:schema-3types}, for the representation
