...
$\land[T_1..\/T_n]$ is the greatest lower bound (abbr. GLB) of
types $T_1..\/T_n$ \cite{Pierce1991,Pierce1996}. The instances of
the type $\land[T_1..\/T_n]$ form a maximal set of objects that
...
types. Therefore, $\lor[T_1..\/T_n]$ is the least upper bound of
types $T_1..\/T_n$ \cite{Pierce1991}.
...
$\{T_1..\/T_n\}$, the meet type $T=\sqcup[T_1..T_n]$ represents a
LUB type such that $T_i\preceq\/T$ for all $i$. On the other hand,
Rule \ref{rul:lor-1} for the $\lor$-types states
$T_i\preceq\lor[T_1..T_n]$. However, the meet type and $\lor$-type
differ in the interpretation. 
$$\llb\lor[T_1..T_n]\rrb_\D=\bigcup_{i\in[1..n]}\llb\/T_i\rrb_\D\subseteq\llb\/T\rrb_\D=\llb\sqcup[T_1..T_n]\rrb_\D$$
...
A join type of $T_1..T_n$ may not exist in a poset of types from a
KG. In general, a join type $J=\sqcap[T_1..T_n]$ exists in a class
...
$J=\sqcap[T_1..T_n]$ includes solely the interpretation of $J$. The
set $\bigcap\/\llb\/T_i\rrb_\D$ can also include objects that are not
instances of $J$. Hence,
$$\llb\sqcap[T_1..T_n]\rrb_\D=\llb\/J\rrb_\D\subseteq\llb\land[T_1..T_n]\rrb_\D=\bigcap_{i\in[1..n]}\/\llb\/T_i\rrb_\D.$$
...
\nnotes{In Yago, often $V$ has a set of very specific classes $C_s$ but also some general classes $C_g$.}
\nnotes{The general classes $C_g$ are close to the classes used in the stored triple types.}
\nnotes{If stored typing of $V$ is correct, then $C_s$ have to include subclasses of $C_g$.}
...
\nnnotes{MIN computes $U_j^{j\in[1,n]}$ such that each $U_j$ is minimal among $T_i^{i\in[1,n]}$.}
\nnnotes{MAX computes $U_j^{j\in[1,n]}$ such that each $U_j$ is maximal among $T_i^{i\in[1,n]}$.}
\nnnotes{Note that in both cases there are no pairs among $U_j^{j\in[1,n]}$ related by $\preceq$.}

\notes{The operator MIN retains from $\land[T_1..T_n]$ only the most specific classes.}
\nnotes{The following is the definition of the operation MIN.}
...
of minimal types $U_i$ of $V$. The types that are important from the
...
user-defined types is the computation of a LUB type of a ground
type. The LUB type of a ground type should be more specific than the
user-defined types that describe the value $V$. If the LUB type is not
more specific than user-defined types then there is an error in stored
types of a value $V$. 

\notes{For $V\in\upsilon$ compute a meet type of $T_1..T_n$ as $V:\sqcup[T_1..T_n]$.}
\nnotes{A meet type is defined on the set of types $\{T_i^{i\in[1,n]}\}$.}
\nnotes{A meet type $T=\sqcup[T_1..T_n]$ is the LUB type of $\{T_i^{i\in[1,n]}\}$ with respect to the relation $\preceq$.}
