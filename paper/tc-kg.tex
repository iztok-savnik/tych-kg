...
predicate in multiple contexts. The context can be defined
differently. As mentioned above, some reasoners (KGs), e.g., Cyc and
Scone, can define and use contextes. A predicate can be defined
differently in different contexts but within one context we can only
have one meaning of a predicate. The context of a predicate can also
be defined in a different way. The senses (contexts) of a predicate
can be defined by means of triple types including a given
predicate. In other words, a sense of a predicate depends on the
context, i.e., a triple type, where the predicate is used. This
approach is similar to the approach used in a natural language for
expressing human contextual reasoning \cite{Fahlman2011}.
...
the use of multiple senses of a predicate. There are many reasons to
use the contextual representation and reasoning in a KG. First, with
the evolvment of KGs there are many examples where the RDF store is
represented in the modular way spliting the dataset into parts that
correspond to the contexts. The meaning of a predicate can be different
in different contexts while the reasoner is able to disambiguate among
the different meanings of a predicate.

There are many ways to define contexts in practical systems. Examples
are the named graphs in KGs DBpedia \cite{Auer2007}, Wiki Data
...
with a function type including, besides the function name, also the
types of its domain and range. Types of function can disambiguate
among the different functions with the same name but different domain
and range types. Similarly, the types can be effectively used to
disambiguate the meaning of the predicate in a KG.

To be able to study KRs including predicates with multiple senses we
propose a minimal KR schema language that includes the triple types in
the form of triples, and OWL type constructors owl:intersectionOf and
owl:unionOf. So far, the union and intersection type constructors are
employed mostly in domain specific KGs like biomedical and genomic
ontologies. In these scientific fields the knowledge includes large
ontologies where new classes can be defined as logical combinations
existing classes.

The KR language that we use is composed of the triple types stored in
a KG as triples. In the case there are more than one triple types
including a predicate $p$ then these are treated as alternatives. For
example, if a KG includes triples $(c_1,p,c_2)$ and $(c_3,p,c_4)$,
where $c_i\in\I_c$, then the type of ground triples including $p$ is
$c_1*p*c_2\lor\/c_3*p*c_4$. In addition, the KR language can use
$\land$ and $\lor$-types in subject and object components of triple
types. The use of $\lor$-type is redundant since it can be expressed
with multiple triple types. Hence, the minimal KR language includes
solely the triple types of the form
