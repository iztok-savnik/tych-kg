...
\notes{We do not have variables with types assigned in $\Gamma$ but facts that are stored in a KG.}
\notes{Things stored in a KG are classified as identifiers and triples, and then further into individual and type objects ($\I$ and $\T$).}
\notes{In this way we obtain individual identifiers $\I_i$ and class identifiers $\I_c$, as well as the individual triples $\T_i$ and triple types $\T_t$.}

\notes{We specify the domains (sets) of \"objects\" (identifiers, triples, classes and types).}
\notes{When we write $O\in\/S$ then we mean the existence of $O$ in a set $S$.}
\notes{We use universal quantification $\forall\/O\in\/S, p(O)$ when we state that some property $p(O)$ holds for all objects $O$ from $S$.}

\notes{Interpretation of the rules.}
\notes{Forward: generative view of rules where rules enumerate the possible conclusions from the premises that generate objects from their domains. We are generating possible results.}
\notes{Backward: for a selected object from a domain we check the construction by means of rule(s).}
