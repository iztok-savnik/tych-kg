...
\nnotes{Rule for filtering $\land[S_1..S_n]$ all $S_i\preceq\/S_k$ by using algorithmic typing.}
\ennotes{We will suppose that there is no subtype relations among $S_i$.}

\begin{equation}
\label{rul:land-2}
\dfrac{I\in\I_i\quad\/I:_\darr\land[S_1..S_n]\quad\/\forall\/i\in[1,n],\ R_j\in\/\{S_k^{k\in[1,n]}\},\ S_i\preceq\/R_j}
      {I:_{\darr}\land[R_1..R_m]}
\end{equation}
...
\nnotes{A join type $S=\sqcup[S_1..S_n]$ is the LUB type of $\{S_i^{i\in[1,n]}\}$.}
\nnotes{$\llb\land[S_1..S_n]\rrb\subseteq\bigcup_{i\in[1..n]}\llb\/S_i\rrb\subseteq\llb\/S\rrb$.}
\nnotes{Let's write here $\sqcup$-type of $\land[S_1..S_m]$!!!}
\ennotes{It can be seen as the operation to compute final type of $I$.}
...
\nnotes{1. type $t:T_g$ is computed bottom-up: from the stored types of triple components.}
\nnotes{2. type $t:T_u$ can be computed top-down: from the user-defined types of properties.}
...
\nnotes{From the top side of the ontology, the stored type $:_\uarr$ is determined based on $p$.}
...
\nnotes{Stored triple types for a given predicate $p$ are computed as MIN of valid stored types for $p$.}
\nnotes{The MIN types of all types obtained from $:_\uarr$ are minimized by using MIN type $:_\Darr$.}
\nnotes{Why type have to be minimal? To have minimal interpretaton.}
