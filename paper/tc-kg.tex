...
$\V_t$. The set of types $\T$ comprises the class identifiers $\T_i$,
i.e. types of individual identifiers $\V_i$, and the triple types
$T_t$, i.e. the types of individual triples $\V_t$.
...
$:_\darr$ that links a value $V\in\V$ to a ground type $T\in\T$. In
this section we deal with the identifiers $I\in\V_i$ which are the
values of type $T\in\T_i$. The ground types of identifiers are types
that are directly linked to the ground identifiers via stored
(explicit) typing relation in the form $(I,rdf:type,T)\in\Delta$. The
expression $I:_\darr\/T$, where $I\in\V_i$ and $T\in\T_i$, states that
a class identifier $T$ is a ground type of an individual identifier
$I$.

The subtyping expression $T_1\preceq_\darr\/T_2$ defines a subtype
relationship between the types $T_1$ and $T_2$ both from $\T$. In the
case we deal with the identifiers, the relation
$T_1\preceq_\darr\/T_2$ denotes the subclass relations since
$T_1,T_2\in\T_i$. The relation $\preceq_\darr$ is a ground subtyping
relation that is stored in $\Delta$. We refer to the relations
$:_\darr$ and $\preceq_\darr$ as \emph{one-step} typing and subtyping
